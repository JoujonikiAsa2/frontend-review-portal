    ...
    <form onSubmit={handleSearch} className="relative">
      <div className={`relative transition-all duration-300 ${isFocused ? 'scale-[1.01]' : ''}`}>
        ...
        <Input
          ...
          className="pl-12 pr-32 py-7 w-full focus:border-none focus:ring-1 focus:ring-black rounded-md shadow-sm text-gray-800 placeholder:text-gray-400 transition-all"
          onFocus={() => setIsFocused(true)}
          onBlur={() => setIsFocused(false)}
        />
        <Button
          type="submit"
          className="absolute right-1.5 top-1/2 transform -translate-y-1/2  text-white font-medium transition-colors group"
        >
          <span>Search</span>
        </Button>
      </div>
